...
(function() {
  ...
  SVGtoWKT.convert = function(svg) {
    ...
    height = Math.round(xml.find("svg").attr("height"))
    width = Math.round(xml.find("svg").attr("width"))
    xml.find("g").each(function(i, tag){
      tr = ($(tag).attr('transform'))
      if(tr.includes("scale")){
        sc = $(tag).attr('transform')
        scale = parseFloat(sc.replace(/s.*,/,'').replace(')',''))
        height /= scale
        width /= scale
      }
    });
    ...
    xml.find('path').each(function(i, path) {
      SVGtoWKT.path($(path).attr('d'));
    });

    for(gg in pt_arrs){
      pt_arrs[gg].unshift(pt_arrs[gg].length)
      els.push(pt_arrs[gg].join("\n"))
    }
  ...
  var __pathPoints = function(path, closed) {
    ...
    _(count+1).times(function(i) {
      ...
      pts.push(String(__round(point.x))+' '+String(__round(height - point.y)));
    });
    pts2 = []
    if (closed) pts.push(pts[0]);
    if(pts.length > 100){
      var incr = Math.ceil(pts.length / 100)
      for(i = 0;i < pts.length;i += incr){
        pts2.push(pts[i])
      }
    } else{
      pts2 = pts
      ...
      var combined = false
      var ee = [[parseInt(pts2[0].split(" ")[0]),parseInt(pts2[0].split(" ")[1])], [parseInt(pts2[pts2.length-1].split(" ")[0]), parseInt(pts2[pts2.length-1].split(" ")[1])]]
      for(i = 0;i < ends.length && !combined;i++){
        for(e in ends[i]){
        for(e2 in ee){
          if(combined){
            break
          }
          var a = ee[e2][0]- ends[i][e][0]
          var b = ee[e2][1]- ends[i][e][1] 
          var dist = Math.sqrt(a*a + b*b) 
          if(dist < 100){
            if(e == 1){
              if(e2 == 0){
                pt_arrs[i] = pt_arrs[i].concat(pts2)
              } else{
                pt_arrs[i] = pt_arrs[i].concat(pts2.reverse())
              }
            } else{
              if(e2 == 0){
                pt_arrs[i] = pts2.reverse().concat(pt_arrs[i])
              } else{
                pt_arrs[i] = pts2.concat(pt_arrs[i])
              }
              
            }
            ends[i] = [[parseInt(pt_arrs[i][0].split(" ")[0]),parseInt(pt_arrs[i][0].split(" ")[1])], [parseInt(pt_arrs[i][pt_arrs[i].length-1].split(" ")[0]), parseInt(pt_arrs[i][pt_arrs[i].length-1].split(" ")[1])]]
